...
function addResponList(groupID, key, response, isImage, image_url, _db) {
  key = key.toLowerCase(); // Pastikan key selalu disimpan dalam huruf kecil
  var obj_add = {
    id: groupID,
    key: key,
    response: response,
...
function getDataResponList(groupId, key, db) {
  key = key.toLowerCase(); // Pastikan pencarian huruf kecil
  return db.find(
    (entry) => entry.id === groupId && entry.key.toLowerCase() === key
  );
}

function isAlreadyResponList(groupId, key, db) {
  key = key.toLowerCase(); // Ubah key yang dicari ke huruf kecil
  return db.some(
    (entry) => entry.id === groupId && entry.key.toLowerCase() === key
  );
}

function sendResponList(groupId, key, db) {
  key = key.toLowerCase();
  let result = db.find(
    (entry) => entry.id === groupId && entry.key.toLowerCase() === key
  );
